...
class ViewService(heartbeat_service_pb2_grpc.ViewServiceServicer):
    ...
    def Heartbeat(self, request, context):
        ...
        log_to_file(f"{service.capitalize()} is alive. Latest heartbeat received at {current_time}")
        return Empty()
    ...
    while True:
        ...
        for service in ["primary", "backup"]:
            ...
            if last_heartbeat and current_time - last_heartbeat > 10:
                ...
                log_to_file(f"{service.capitalize()} might be down. Latest heartbeat received at {timestamp}")
...
def serve():
    ...
    monitoring_thread = threading.Thread(target=monitor_heartbeats, daemon=True)
